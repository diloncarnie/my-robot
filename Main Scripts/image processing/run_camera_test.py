...
def run_ocr(snapshot):
    ...
    print("Saved OCR detection image to ocr_detection_output.jpg")

    
    return sorting_letter, assigned_index

def main():
    print('OCR Test Program Started')
    ...
    print("LIVE OCR FEED ACTIVE")


    snapshot = get_vision_sensor_snapshot(sim, vision_sensor)
    sorting_letter, assigned_index = run_ocr(snapshot)
    print(f"SORTED LETTER: {sorting_letter} | ASSIGNED EVELOPE INDEX: {assigned_index}")
