...
def reset_objects(sim):
    """Resets all simulated objects to a clean static state."""
    print("Resetting all objects to static state...")
    for name in SIMULATED_OBJECTS:
        try:
            handle = sim.getObject(name)
            sim.setObjectParent(handle, -1, True)
            sim.setObjectInt32Param(handle, 3003, 1) # Static
            sim.setObjectInt32Param(handle, 3004, 1) # Respondable
        except:
            pass
...
def main(robot: ERobot) -> None:
    ...
    try:
        while True:
            sim.setStepping(False) # Let simulation run freely in background
            snapshot = get_vision_sensor_snapshot(sim, vision_sensor)
            sorting_letter, assigned_index = run_ocr(snapshot)
            sim.setStepping(True)  # Resume synchronous control for precise robot movement
            
            if sorting_letter is None or assigned_index is None:
                print("No detected letters. Stopping simulation.")
                break
            ...
            w = sim.getObjectPose(sim.getObject(index_wp), sim.handle_world) #Go to pickup position for selected Envelope
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=2.0)
            set_gripper(sim, gripper_joint, open_state=False, object_name=envelope_name) #Pickup selected Envelope
            w = sim.getObjectPose(sim.getObject(index_buffer_wp), sim.handle_world) #Go to buffer position for selected Envelope
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=3.5, straight_line=True)
            
            w = sim.getObjectPose(sim.getObject(default_position), sim.handle_world) #Go to default position
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=2.0)
            
            w = sim.getObjectPose(sim.getObject(pigeonhole_wp), sim.handle_world) #Go to default position for selected pigeon hole
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=2.0)
            w = sim.getObjectPose(sim.getObject(pigeonhole_buffer_wp), sim.handle_world) #Go to buffer position for selected pigeon hole
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=2.0, straight_line=True)
            set_gripper(sim, gripper_joint, open_state=True, object_name=envelope_name, make_dynamic=True) #Drop the selected envelope into the pigeon hole

            w = sim.getObjectPose(sim.getObject(default_position), sim.handle_world) #Go to default position
            current_q = move_to_pose(sim, robot, joints, w, current_q, max_vel, max_accel, max_jerk, duration_s=2.0)
        
        _ =  (current_q, max_vel, max_accel, max_jerk)
